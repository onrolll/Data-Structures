...
public class BinarySearchTree<T> : IBinarySearchTree<T> where T : IComparable
{
    ...
    {
        if (Count(root.Left) > rank)
        {
            return this.Select(rank, root.Left);
        }
        else if(Count(root.Left) < rank)
        {
            return this.Select(rank - 1 - Count(root.Left), root.Right);
        }
        return root;
    ...
    private Node Floor(T element, Node node)
    {
        ...
        else
        {
            ...
            var res = Floor(element, node.Right);
            if (res == null)
                return node;
            return res;
        }
